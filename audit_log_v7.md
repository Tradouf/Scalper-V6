...
## 2026-07-04 03:00 (audit Opus V7)
**Métriques 6h** : szi0_frozen=0, emergency=0, drift=0, breakout=0, errors=3 (1 HTTPError/1 Error/1 error, mêmes événements 429), equity=$992.05→$990.02
**Diagnostic** : Fenêtre saine. Régime 100% trend_up (720 ticks), toutes pathologies grille + emergency + drift/breakout à 0. Équity quasi-plate (-$2.03, -0,20%) sur la base ~$991 (cf. [[project_phantom_drawdown_withdrawals]]) → pas de saignement, la rotation tient. Les 3 erreurs = bruit réseau (HTTP 429 candles AAVE 1h sur hl_adapter ; aucun type ≥50) — couvert par la proposition pending 06-08 (retry/backoff hl_adapter), ne pas re-proposer. Storm submit XRP toujours résolu (0 HyperliquidClientError cette fenêtre, cf. [[project_xrp_zec_min_notional_storm]] fix 8666c96). Stratégies bornées (grid/MR/momentum/supertrend) désactivées (all-in rotation hors allocateur) → aucun levier paramètre ici.
**Changes** : aucun
**Code proposals** : aucune
**Alerts** : aucun
...
**Changes** : aucun
**Code proposals** : aucune
**Alerts** : aucun (storm XRP submit confirmé résolu par 8666c96 sur fenêtre 100% post-restart)

## 2026-07-02 15:00 (audit Opus V7)
**Métriques 6h** : szi0_frozen=0, emergency=0, drift=0, breakout=0, errors=273 (272 HyperliquidClientError = mêmes événements 'error'), equity=$1000.39→$996.12
**Diagnostic** : Fenêtre saine côté trading. Régime 100% range (720 ticks), toutes pathologies grille + emergency + drift/breakout à 0, équity quasi-plate (-$4.27, -0,43%) sur la base ~$1000 (cf. [[project_phantom_drawdown_withdrawals]]) → pas de saignement, la rotation tient. Les 272 HyperliquidClientError franchissent le seuil ≥50, MAIS ce sont **tous** des `ExecutionEngine submit error XRP buy` jusqu'à **11:15:56**, immédiatement suivis d'un `BootReconciler résumé` à **11:16:17** (positions=4 adoptées=0 equity=$999.65) → signature d'un **RESTART à 11:16 déployant le fix min-notional commit 8666c96** ($10→$11, cf. [[project_xrp_zec_min_notional_storm]]). Le storm est donc le RÉSIDU pré-fix (moitié gauche de la fenêtre), pas une pathologie active : bug déterministe min-notional déjà corrigé et vérifié live (0 ERROR post-restart). Famille déjà couverte par 3 propositions pending (06-02 log complet, 06-15 anti-boucle, 07-02 09:00 circuit-breaker) → pas de re-proposition. Aucun levier paramètre : stratégies bornées (grid/MR/momentum/supertrend) désactivées (all-in rotation hors allocateur), XRP buy non réglable. Pas de commit yaml (le bot vient de redémarrer à 11:16, inutile d'ajouter un downtime).
**Changes** : aucun
**Code proposals** : aucune (storm XRP = résidu pré-fix, corrigé par 8666c96 ; famille couverte par 3 pending)
**Alerts** : aucun critique — les 272 erreurs sont le résidu du storm XRP AVANT le restart de 11:16 qui a déployé le fix min-notional (8666c96). À CONFIRMER au prochain audit : 0 `submit error XRP` sur une fenêtre entièrement post-restart (la mémoire indique déjà 0 ERROR vérifié live).
...
**Changes** : aucun
**Code proposals** : aucune
**Alerts** : aucun (dépôt capital ~$399 ; NB la rotation va se redimensionner sur la nouvelle equity $760 — à $760 les 8 coins repassent largement au-dessus du min HL $10, déploiement plus large attendu, à surveiller au prochain audit)
...
## 2026-06-27 03:00 (audit Opus V7)
**Métriques 6h** : szi0_frozen=0, emergency=0, drift=0, breakout=0, errors=2 (1 ReadTimeoutError/1 error), equity=$421.29→$421.16
**Diagnostic** : Fenêtre saine. Régime 100% range (720 ticks), toutes pathologies grille + emergency + drift/breakout à 0. Équity quasi-plate (-$0.13, -0,03%). 2 erreurs = bruit réseau (1 ReadTimeout candles LINK 1h sur hl_adapter, aucun type ≥50). Stratégies bornées (grid/MR/momentum/supertrend) désactivées (all-in rotation hors allocateur) → aucun levier paramètre ici.
**Changes** : aucun
**Code proposals** : aucune
**Alerts** : aucun
...
## 2026-06-25 09:00 (audit Opus V7)
**Métriques 6h** : szi0_frozen=0, emergency=0, drift=0, breakout=0, errors=3 (1 HTTPError/1 Error/1 error), equity=$420.77→$419.64
**Diagnostic** : Fenêtre saine. 100% high_vol (720 ticks), toutes pathologies grille + emergency à 0. Équity quasi-plate (-$1.13, -0,27%). 3 erreurs = bruit réseau (1 HTTP 429 candles ETH 1h, <50). Stratégies bornées (grid/MR/momentum/supertrend) désactivées (all-in rotation hors allocateur) → aucun levier paramètre ici.
**Changes** : aucun
**Code proposals** : aucune
**Alerts** : aucun
...
## 2026-06-23 15:00 (audit Opus V7)
**Métriques 6h** : szi0_frozen=0, emergency=0, drift=0, breakout=0, errors=11 (4 error/3 ReadTimeoutError/2 ConnectionError/1 NameResolutionError/1 MaxRetryError), equity=$417.90→$418.98 (+$1.08 / +0,26 %)
**Diagnostic** : Fenêtre calme. Régime 100 % range (717 ticks), toutes pathologies grille + emergency à 0, équity en légère hausse sur capital stable ~$417. Erreurs = bruit réseau transitoire sur refresh HL (candles TAO/ZEC 1h, allMids), 11 total largement <50, aucun type ≥50 (couvert par pending 06-08). Stratégies bornées (grid/MR/momentum/supertrend) toutes `enabled:false` (all-in rotation hors allocateur) → leviers paramètres inertes.
**Changes** : aucun
**Code proposals** : aucune (6 pending inchangées — pas de re-proposition)
**Alerts** : aucun

## 2026-06-23 21:00 (audit Opus V7)
**Métriques 6h** : szi0_frozen=0, emergency=0, drift=0, breakout=0, errors=0, equity=$418.97→$419.15 (+$0.18 / +0,04 %)
**Diagnostic** : Fenêtre calme. Toutes pathologies grille + emergency + errors à 0, équity quasi-plate sur capital stable ~$419. Régime 99 % trend_down (712/720), 8 ticks range. Stratégies bornées (grid/MR/momentum/supertrend) toutes `enabled:false` — rotation ensemble-contrarian 1d pilote hors allocateur → leviers paramètres inertes. Aucun pattern actionnable.
**Changes** : aucun
**Code proposals** : aucune (6 pending inchangées — pas de re-proposition)
**Alerts** : aucun

## 2026-06-24 03:00 (audit Opus V7)
**Métriques 6h** : szi0_frozen=0, emergency=0, drift=0, breakout=0, errors=0, equity=$419.15→$418.37 (-$0.78 / -0,19 %)
**Diagnostic** : Régime 100 % trend_down (720 ticks). Fenêtre calme, aucun pattern net. Toutes pathologies grille à 0 (szi0/abandons/RO/DRIFT/BREAKOUT/doublons/activations), 0 EMERGENCY EXIT, 0 erreur. Équity quasi-plate sur capital stable ~$418. Stratégies bornées (grid/MR/momentum/supertrend) toutes `enabled:false` — rotation ensemble-contrarian 1d pilote hors allocateur → leviers paramètres inertes. Aucun paramètre actionnable.
**Changes** : aucun
**Code proposals** : aucune (6 pending couvrent réseau/emergency/submit-exit/NameError/freins-catastrophe/runaway HYPE — pas de re-proposition)
**Alerts** : aucun

## 2026-06-24 09:00 (audit Opus V7)
**Métriques 6h** : szi0_frozen=0, emergency=0, drift=0, breakout=0, errors=10 (2 ProtocolError/2 error/2 ConnectionResetError/2 ConnectionError/1 HTTPError/1 Error), equity=$418.34→$418.44 (+$0.10 / +0,02 %)
**Diagnostic** : Régime 100 % trend_down (720 ticks). Fenêtre calme, aucun pattern net. Toutes pathologies grille à 0 (szi0/abandons/RO/DRIFT/BREAKOUT/doublons/activations), 0 EMERGENCY EXIT. 10 erreurs = bruit réseau transitoire sur fetch candles 1h (DOGE/TAO), dont 1 HTTP 429 (rate-limit ponctuel) — très en dessous du seuil ≥50. Équity strictement plate sur capital stable ~$418. Stratégies bornées (grid/MR/momentum/supertrend) toutes `enabled:false` — rotation ensemble-contrarian 1d pilote hors allocateur → leviers paramètres inertes. Aucun paramètre actionnable.
**Changes** : aucun
**Code proposals** : aucune (6 pending couvrent réseau/emergency/submit-exit/NameError/freins-catastrophe/runaway HYPE — pas de re-proposition)
**Alerts** : aucun
