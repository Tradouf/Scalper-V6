"""
Filtre qualité des symboles SuperBot — logique reprise de simplebot/symbol_filter.py
(SPEC §12), branchée sur la config SUPERBOT_* (seuils et cap indépendants).

  1. allowlist / blocklist explicites ;
  2. seuils qualité stricts (PF/PnL/WR valid + PF/PnL train) ;
  3. plafond MAX_ACTIVE_SYMBOLS : top-N par score composite.

Les symboles déclassés restent dans best_params.json (active=False) avec
`filter_reason` distinct du `reason` optimiseur (traçabilité dashboard).
"""
...
logger = logging.getLogger("sdm.superbot.symbol_filter")
...
def check_quality_gate(symbol: str, entry: dict) -> Tuple[bool, str]:
    """(ok, reason) pour un symbole déjà actif côté walk-forward."""
    sym = symbol.upper()
...
def apply_symbol_filter(per_symbol: Dict[str, dict]) -> Dict[str, dict]:
    """Applique le filtre sur le dict symbols de l'optimiseur (copie défensive)."""
    ...
    if cap > 0:
        ...
        if len(active) > cap:
            ...
            logger.info(
                ...
                cap, sorted(keep), [sym for sym, _ in active[cap:]],
            )
